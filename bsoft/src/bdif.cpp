...
Vector3<double>	img_analyze_reflections(Bimage* p, double ref_res, double threshold, long kedge, int sym);
//Vector3<double>	img_analyze_reflection(Bimage* p, double ref_res, double threshold, long kedge, int sym);
...
int 	main(int argc, char* argv[])
{
	...
	if ( ref_res ) img_analyze_reflections(p, ref_res, threshold, kedge, sym);
...
Vector3<double>	img_analyze_reflections(Bimage* p, double ref_res, double threshold, long kedge, int sym)
{
	vector<Vector3<double>>	loc = img_find_reflections(p, ref_res, threshold, kedge);
	loc = symmetrize_reflections(loc, sym, kedge/p->real_size()[0]);
	return analyze_reflections(loc, 1.0/ref_res);
}
...
//		cout << na << tab << tloc << tab << rloc << endl;
	...
	if ( verbose ) {
		...
		cout << "Relative magnification:         " << res_avg/ref_res << endl;
		cout << "Scale in u and v:               " << f2 << endl << endl;
		cout << "Input pixel size:               " << p->sampling(0) << endl;
		cout << "Estimated pixel size:           " << sam << endl << endl;
	}
