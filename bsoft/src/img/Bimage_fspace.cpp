...
long		Bimage::fspace_maximum_radius(double resolution, double sampling_ratio)
{
	...
	if ( resolution > 0.1 )
		maxrad = (long) (rad_scale/resolution+1);
	else
...
/**
@brief 	Rotates an image in frequency space with kernel-based interpolation.
@param 	mat				3x3 rotation matrix.
@param 	*kernel			frequency space interpolation kernel.
@return Bimage*			new 2D central section.

	The interpolation is done with a reciprocal space kernel.
	The rotation origin is the standard transform origin.

**/
Bimage*		Bimage::fspace_rotate(Matrix3 mat, FSI_Kernel* kernel)
{
	...
	if ( verbose & VERB_FULL ) {
		cout << "Rotating in frequency space:" << endl;
		cout << mat << endl << endl;
	}

	Bimage*			pnu = new Bimage(Float, TComplex, size(), 1);
	pnu->fourier_type(Standard);
	pnu->image[0] = image[0];
	pnu->sampling(image->sampling());
	pnu->origin(image->origin());
	...
	for ( i=zz=0; zz<y; ++zz ) {
		...
		for ( yy=0; yy<y; ++yy ) {
			...
			for ( xx=0; xx<x; ++xx, ++i ) {
				...
				m = mat * iv;
				m *= real_size();
				pnu->set(i, fspace_interpolate(0, m, kernel));
			}
		}
	}
...
int			Bimage::fspace_bandpass(double res_hi, double res_lo, double width)
{
	...
	planf = fft_setup(FFTW_FORWARD, 0);
	planb = fft_setup(FFTW_BACKWARD, 0);
...
int 		Bimage::fspace_weigh_RPS_curve(Bplot* plot, double resolution)
{
	...
	if ( verbose & VERB_DEBUG ) {
		cout << "DEBUG fspace_weigh_RPS_curve: rad_scale=" << rad_scale << endl;
		cout << "DEBUG fspace_weigh_RPS_curve: (*plot)[1]=" << (*plot)[1] << endl;
		cout << "DEBUG fspace_weigh_RPS_curve: scale=" << scale << endl;
	}
...
/**
@brief 	Calculates the phase difference between two images.
...
//	ppd->fft();
	...
	Vector3<double>	scale(1.0/p->real_size());
	Complex<float>	temp1, temp2;
...
//		ppd->image[nn].origin(image[nn].origin() - p->image[nn].origin());
		shift = p->image[nn].origin()/p->size();
		if ( verbose & VERB_FULL )
			cout << nn+1 << tab << shift << endl;
		for ( zz=0; zz<p->z; ++zz ) {
			iz = -zz;
			...
			for ( yy=0; yy<p->y; ++yy ) {
				...
				if ( iy < 0 ) iy += p->y;
				dy = yy;
				if ( yy > (p->y - 1)/2 ) dy -= p->y;
				sy2 = dy*scale[1];
				sy2 *= sy2;
				for ( xx=0; xx<p->x/2+1; ++xx, ++i ) {
					ix = -xx;
					if ( ix < 0 ) ix += p->x;
					sx2 = xx*scale[0];
					sx2 *= sx2;
...
//						dphi = temp1.phi() - temp2.phi() - TWOPI*(xx*shift[0] + yy*shift[1] + zz*shift[2]);
						dphi = p1->complex(i).phi() - p2->complex(i).phi() - TWOPI*(xx*shift[0] + yy*shift[1] + zz*shift[2]);
						dphi = angle_set_negPI_to_PI(dphi);
						switch ( weighting ) {
							case 1:
								amp = temp1.amp();
