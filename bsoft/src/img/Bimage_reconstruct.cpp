...
int			Bimage::fspace_pack_2D(Bimage* p, Matrix3 mat, double hi_res, 
				...
				double part_weight, int interp_type)
{
	if ( hi_res < 0.1 ) hi_res = 0.1;	// Limit on resolution!
...
//	cout << setprecision(4) << p->real_size() << tab << invscale << tab << mscale << endl;
	...
	if ( verbose & VERB_FULL ) {
		...
		cout << "Ewald sphere wavelength = " << setprecision(4) << ewald_wavelength << endl;
		if ( p2 ) cout << "Correcting with a complex CTF" << endl;
	}
		...
		for ( xx=0; xx<p->x; ++xx, ++i ) {
			...
			if ( s2 >= mins2 && s2 <= maxs2 ) {
				...
				if ( ew ) {
					...
					m = mat * s;
					m *= mscale;
					if ( p2 ) fspace_2D_interpolate(p2->complex(i), m, w, interp_type);
					else fspace_2D_interpolate(p->complex(i), m, w, interp_type);
				}
			}
		}
	}
...
**/
int			Bimage::fspace_pack_2D(Bimage* p, View2<double> asu_view, Bsymmetry& sym, double hi_res,
				double lo_res, Vector3<double> scale, double ewald_wavelength, double part_weight, int interp_type)
{
	vector<View2<double>>	views = sym.get_all_views(asu_view);
