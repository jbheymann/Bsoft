...
/**
@brief 	Calculates the occupied volume above a given threshold.
@param 	threshold		threshold value.
@return double			volume in angstrom cubed.

**/
double		Bimage::occupied_volume(double threshold)
{
	double		vox_size(sampling(0).volume());
	double		volume(0);
	
	for ( long i=0; i<data_size(); ++i )
		if ( (*this)[i] > threshold ) volume += vox_size;
