/**
@file	mg_ctf_fit.cpp
@brief	Functions for CTF (contrast transfer function) fitting
@author 	Bernard Heymann
@date	Created: 19970715
@date	Modified: 20231106
**/
...
double		ctf_fit_baseline_zeroes(Bimage* prad, double real_size, CTFparam& em_ctf, double lores, double hires);
double		ctf_fit_envelope_zeroes(Bimage* prad, double real_size, CTFparam& em_ctf, double lores, double hires);
double		ctf_fit_exp_decay(Bimage* prad, double real_size, CTFparam& em_ctf, double lores, double hires);
double		ctf_fit_constant_baseline(Bimage* prad, CTFparam& em_ctf);
...
Bimage*		img_ctf_radial_average(Bimage* p, long n, CTFparam& em_ctf)
{
	...
	if ( verbose & VERB_FULL ) {
		...
		cout << "Size:                           " << nrad << endl;
		cout << "Power spectrum origin:          " << p->image[n].origin() << endl;
...
int			img_manage_power_spectrum_peak(Bimage* p, Bimage* pb, double radius)
{
	long		i, xx, yy, zz;
	double		dx, dy, f;
...
*/
Bimage*		img_ctf_fit_prepare(Bimage* p, double tmax, double sigma)
{
	long		gauss_kernel(6*sigma);
//	long		radius(p->real_size()[0]/30);
//	double		radius(5);
	...
	Bimage*		pt = p->copy();
	pt->truncate_to_min_max(0,tmax);				// Ensure within [0,100]
...
//	if ( verbose & VERB_DEBUG )
		write_img("image1.mrc", pb, 0);

	Bimage*		pe = pb->copy();
	pe->add(pt, -1, 0);							// Subtract B + E/2 to get ~S/2
//	if ( verbose & VERB_DEBUG )
		write_img("image2.mrc", pe, 0);
...
Bimage*		img_ctf_fit_prepare(Bimage* p, long n, double tmax, double sigma)
{
	Bimage*		ps = p->extract(n);

	Bimage*		pf = img_ctf_fit_prepare(ps, tmax, sigma);
...
*/
	...
	double 			fom(0), tmax(20), sigma(10);
	Bimage*			pf = NULL;
	if ( flag & 32 ) {
		pf = img_ctf_fit_prepare(p, n, tmax, sigma);
...
double		img_ctf_fit_baseline(Bimage* p, long n, CTFparam& em_ctf, double lores, double hires)
{
	...
	if ( em_ctf.baseline_type() )
		R = ctf_fit_baseline_zeroes(prad, p->real_size()[0], em_ctf, lores, hires);
	else
		R = ctf_fit_constant_baseline(prad, em_ctf);
...
double		img_ctf_fit_envelope(Bimage* p, long n, CTFparam& em_ctf, double lores, double hires)
{
	...
	if ( em_ctf.envelope_type() < 5 )
		R = ctf_fit_envelope_zeroes(prad, p->real_size()[0], em_ctf, lores, hires);
	else
		R = ctf_fit_exp_decay(prad, p->real_size()[0], em_ctf, lores, hires);
...
/**
@brief 	Fits the baseline and envelope for a given CTF.
@param 	*p			image structure.
@param	n			sub-image number.
@param 	&em_ctf		CTF parameter structure.
@param 	lores		low resolution limit.
...
**/
double		img_ctf_fit_baseline_envelope(Bimage* p, long n, CTFparam& em_ctf, double lores, double hires)
{
//	if ( verbose & VERB_DEBUG )
		cout << "DEBUG: img_ctf_fit_baseline_envelope" << endl;

	Bimage* 	prad = img_ctf_radial_average(p, n, em_ctf);
	
	if ( !prad ) {
		cerr << "Error in img_ctf_fit_baseline_envelope: No radial power spectrum calculated!" << endl;
		return -1;
	}
	
	em_ctf.baseline_type(0);
	em_ctf.envelope_type(6);
	
	double		R = ctf_fit_exp_decay(prad, p->real_size()[0], em_ctf, lores, hires);
...
//	if ( verbose & VERB_DEBUG )
		cout << "DEBUG: img_ctf_fit_baseline_envelope: done" << endl;
...
int			img_add_aberration_terms(Bimage* psum, Bimage* psec, Bimage* p, CTFparam& cp, bool ew_flag)
{
	long 			i, j, n, x, y, z;
	double			sx, sy, sz, s, a, dphi, pwr;
	...
	for ( i=j=n=0; n<psum->images(); n++ ) {
		for ( z=0; z<psum->sizeZ(); z++ ) {
			...
			for ( y=0; y<psum->sizeY(); y++ ) {
				...
				sy *= freq_scale[1];
				for ( x=0; x<psum->sizeX(); x++, i++ ) if ( psec->complex(i).not_zero() ) {
					sx = x;
					...
					if ( !ew_flag ) {	// Original
						s = sx*sx + sy*sy + sz*sz;
						dphi = cp.calculate_aberration_even(s, a);
						ca = Complex<double>(cosl(dphi),sinl(dphi));
					} else {			// Ewald sphere
						s = sqrt(sx*sx + sy*sy + sz*sz);
						ca = cp.calculate_complex(s, a);
					}
					cv = psec->complex(i);
					pwr = cv.power();
					cv *= p->complex(i).conj();
...
Bimage*		img_calculate_phase_differences(Bimage* psum, int check_contrast)
{
	long			i, j, k, n;
	double			ae, ao;
	vector<double>	t(2);
	...
	int				contrast(1);
	if ( check_contrast ) contrast = img_check_contrast(psum);
	...
	for ( n=i=j=0; n<pphi->images(); ++n ) {
		for ( k=0; k<pphi->image_size(); ++i, ++k, j+=psum->channels() ) if ( (*psum)[j] ) {
			t[0] = (*psum)[j+2];
			t[1] = (*psum)[j];
			r[0][0] = (*psum)[j+3];
...
{
	if ( hires < p->image[nn].sampling()[0] ) hires = p->image[nn].sampling()[0];
	...
	double			s2lo = (lores)? 1/(lores*lores): 0;
	long			nt(15);
	if ( flag == 1 ) nt = 6;
	else if ( flag == 2 ) nt = 9;
	...
	for ( i=nn*p->image_size(), yy=0; yy<p->sizeY(); ++yy ) {
		v = (double(yy) - p->image[nn].origin()[1])/p->sizeY();
		if ( v >= 0.5 ) v -= 1;
		v /= p->image[nn].sampling()[1];
		for ( xx=0; xx<p->sizeX(); ++xx, ++i ) {
			u = (double(xx) - p->image[nn].origin()[0])/p->sizeX();
			if ( u >= 0.5 ) u -= 1;
			u /= p->image[nn].sampling()[0];
			s2 = u*u + v*v;
			if ( s2 >= s2lo && s2 <= s2hi ) {
//				if ( flag == 1 ) t = aberration_odd_terms(nt,u,v);
//				else if ( flag == 2 ) t = aberration_even_terms(nt,u,v);
//				else t = aberration_terms(nt,u,v);
				t = aberration_terms(u,v,flag);
				if ( t.size() != nt )
					cerr << "Error in img_aberration_phase_fit: The number of aberration terms is incorrect! (" << t.size() << "!=" << nt << ")" << endl;
				for ( j=0; j<nt; ++j ) {
					b[j] += t[j]*(*p)[i];
					for ( k=0; k<nt; ++k )
						A[j][k] += t[j]*t[k];
...
double		img_aberration_phase_fit_iter_old(Bimage* p, long nn, double lores, double hires, int flag, long iter, map<pair<long,long>,double>& wa)
{
	...
	for ( i=nn*p->image_size(), yy=0; yy<p->sizeY(); ++yy ) {
		...
		if ( s[1] >= 0.5 ) s[1] -= 1;
		s[1] /= p->image[nn].sampling()[1];
		for ( xx=0; xx<p->sizeX(); ++xx, ++i ) {
			s[0] = (double(xx) - p->image[nn].origin()[0])/p->sizeX();
			if ( s[0] >= 0.5 ) s[0] -= 1;
			s[0] /= p->image[nn].sampling()[0];
			slen = s.length();
...
double		img_aberration_phase_fit_iter(Bimage* p, long nn, double lores, double hires, int flag, long iter, map<pair<long,long>,double>& wa)
{
	...
	for ( i=nn*p->image_size(), yy=0; yy<p->sizeY(); ++yy ) {
		...
		for ( xx=0; xx<p->sizeX(); ++xx, ++i ) {
			...
			if ( s2 >= s2lo && s2 <= s2hi ) {
				phases.push_back((*p)[i]);
				t = aberration_terms(wa, u, v);
				if ( t.size() != nt )
					cerr << "Error in img_aberration_phase_fit_iter: The number of aberration terms is incorrect! (" << t.size() << "!=" << nt << ")" << endl;
				for ( j=0; j<nt; ++j ) terms.push_back(t[j]);
			}
		}
	}
...
vector<map<pair<long,long>,double>>	img_aberration_phase_fit(Bimage* p, double lores, double hires, int flag,
		long iter)
{
	vector<map<pair<long,long>,double>>		win(p->images());
...
#pragma omp parallel for
	...
	if ( verbose ) {
		...
		for ( long nn=0; nn<p->images(); ++nn ) {
			ws = aberration_weight_string(win[nn]);
...
double		exp_decay_R(Bsimplex& simp)
{
	long			i;
	double			R(0), df;
	vector<double>&	f = simp.dependent_values();
	...
	for ( i=0; i<simp.points(); i++ ) {
		df = f[i] - (simp.parameter(0) + simp.parameter(1)*pow(x[i], simp.parameter(2)));
		R += df*df;
	}
...
double		ctf_fit_baseline_zeroes(Bimage* prad, double real_size, CTFparam& em_ctf, double lores, double hires)
{
	...
	if ( verbose & VERB_DEBUG ) {
		cout << "DEBUG ctf_fit_baseline_zeroes: zeroes=" << n << endl;
		for ( auto z: zero ) cout << z << endl;
	}
	
	double			min(1e10), dmin;
	
	if ( em_ctf.baseline_type() < 1 ) {			// Constant baseline
		fill(coeff.begin(), coeff.end(), 0);
		coeff[0] = prad->minimum();
		R = 0;
	} else if ( n > 3 ) {		// Enough zeroes to fit the baseline
		for ( i=0; i<n; i++ ) {
	...
	if ( verbose & VERB_DEBUG ) {
		cout << "DEBUG ctf_fit_baseline_zeroes: type " << em_ctf.baseline_type() << endl;
...
{
	if ( verbose & VERB_DEBUG )
		cout << "DEBUG ctf_fit_envelope_zeroes" << endl;
	...
	if ( nzero < 5 ) {
		cerr << "Error in ctf_fit_envelope_zeroes: Cannot fit with fewer than 5 zeroes!" << endl;
		cerr << tab << "Defocus = " << em_ctf.defocus_average() << endl;
		return 0;
	}
	
	if ( verbose & VERB_DEBUG )
		cout << "DEBUG ctf_fit_envelope_zeroes: nzero=" << nzero << endl;
	...
	if ( verbose & VERB_DEBUG ) {
		cout << "DEBUG ctf_fit_envelope_zeroes: type " << em_ctf.envelope_type() << endl;
		cout << "Coefficients:" << endl;
	...
	if ( verbose & VERB_DEBUG ) {
		cout << "DEBUG ctf_fit_envelope_zeroes:";
...
double		ctf_fit_constant_baseline(Bimage* prad, CTFparam& em_ctf)
{
	...
	em_ctf.baseline_type(0);
	em_ctf.baseline(0, mavg.back());

	return R;
}

/*
@brief 	Fits a near hyperbole function.
@param 	*prad		radial power spectrum.
@param 	real_size	real size in angstrom.
@param 	&em_ctf		CTF parameters.
@param	lores		low resolution limit.
@param	hires		high resolution limit.
@return double 		R value.

	Near hyperbole function:
		y = a + b*s^c
**/
double		ctf_fit_exp_decay(Bimage* prad, double real_size, CTFparam& em_ctf, double lores, double hires)
{
	...
	if ( verbose & VERB_DEBUG ) {
		cout << "DEBUG ctf_fit_exp_decay: real_size=" << real_size << endl;
		cout << "DEBUG ctf_fit_exp_decay: smin=" << smin << " smax=" << smax << endl;
	}
...
//		if ( s1 >= smin && s1 <= smax ) {
		if ( s1 >= smin ) {
			s.push_back(s1);
			d.push_back((*prad)[i] - em_ctf.calc_baseline(s1));
		}
	}

//	if ( verbose & VERB_DEBUG )
		cout << "DEBUG ctf_fit_exp_decay: s.size()=" << s.size() << endl;
		cout << "DEBUG ctf_fit_exp_decay: d.back()=" << d.back() << endl;

	Bsimplex		simp(1, 3, 0, s.size(), s, d);
	
	if ( d.back() > 0 ) simp.parameter(0, d.back());
	if ( simp.parameter(0) ) simp.limits(0, 0, 2*simp.parameter(0));
	else simp.limits(0, 0, 100);
	simp.limits(1, (s[0] - simp.parameter(0))/10, 5*(s[0] - simp.parameter(0)));
	simp.limits(2, -2, -1);
	
	double			R = simp.run(10000, 0.01, exp_decay_R);
	
	em_ctf.envelope(0, simp.parameter(0));
	em_ctf.envelope(1, 2*simp.parameter(1));
	em_ctf.envelope(2, simp.parameter(2));

	if ( verbose & VERB_DEBUG ) {
		cout << "DEBUG ctf_fit_exp_decay: e0=" << em_ctf.baseline(0) << endl;
		cout << "DEBUG ctf_fit_exp_decay: e1=" << em_ctf.envelope(1) << endl;
		cout << "DEBUG ctf_fit_exp_decay: e2=" << em_ctf.envelope(2) << endl;
		cout << "DEBUG ctf_fit_exp_decay: R=" << R << endl;
	}
