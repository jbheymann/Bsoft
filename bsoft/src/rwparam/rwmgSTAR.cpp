...
//	ctf->astigmatism_angle(block.real(CTF_AST_ANG) * M_PI/180.0);
	...
	long		i, j;
	for ( auto il: block.loops() ) {
		if ( ( i = il.find(CTF_ABERRATION_N) ) >= 0 ) {
			long		n(0), m(0);
			for ( auto ir: il.data() ) {
				if ( ( j = il.find(CTF_ABERRATION_N) ) >= 0 )
					n = to_integer(ir[j]);
				if ( ( j = il.find(CTF_ABERRATION_M) ) >= 0 )
					m = to_integer(ir[j]);
				if ( ( j = il.find(CTF_ABERRATION_COEFFICIENT) ) >= 0 )
					ctf->aberration_coefficient(n, m, to_real(ir[j]));
			}
		}
	}
...
int			star_to_project(Bstar& star, Bproject* project, int flag)
{
	...
	if ( verbose & VERB_PROCESS )
		cout << nfield << " fields, " << nmg << " micrographs, " << nrec << " reconstructions converted" << endl;
...
*/
	BstarLoop&		loop = block.add_loop();
	loop.tags()[CTF_ABERRATION_N] = 0;
	loop.tags()[CTF_ABERRATION_M] = 1;
	loop.tags()[CTF_ABERRATION_COEFFICIENT] = 2;

	map<pair<long,long>,double>	ca = ctf->aberration_coefficients();
	
	for ( auto c: ca ) {
		vector<string>&	vs = loop.add_row(3);
		vs[0] = to_string(c.first.first);
		vs[1] = to_string(c.first.second);
		vs[2] = to_string(c.second);
	}
