...
//		}
		...
		if ( curropt->tag == "timestep" )
			if ( ( md.timestep = curropt->value.real() ) < 1e-30 )
				cerr << "-timestep: The time step must be specified!" << endl;
		if ( curropt->tag == "velocitylimit" )
			if ( ( md.velocitylimit = curropt->value.real() ) < 1e-30 )
...
//	model_generate_angles(waters);
	...
	double			max_shift(1);
	if ( max_iter ) {
		model_mechanics(waters, md, 0, max_iter, max_shift);
		if ( verbose & VERB_PROCESS )
			models_density(waters);
		if ( verbose )
			model_calculate_deviations(waters, md);
	...
	if ( rdf > 0 ) model_calc_water_rdf(waters, rdf, md.cutoff);
