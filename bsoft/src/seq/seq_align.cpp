...
@return pair<string,string>	length of the aligned sequences.

	Two sequences are aligned and returned as a pair of aligned strings.
...
//	if ( verbose & VERB_DEBUG )
		cout << "DEBUG seq_pair_align:"  << endl << seq1.length() << tab << seq1 << endl 
			<< seq2.length() << tab << seq2 << endl;
	
	Matrix			dp;
	
	dp = seq_dot_plot(seq1, seq2, simat);
	
	long			imax = dp_matrix_scoring(dp, gapopen, gapextend);
	
	long			i(imax/dp.columns()), j(imax%dp.columns());
//	cout << dp.rows() << tab << dp.columns() << endl;
//	cout << imax << tab << i << tab << j << endl;
	double			score = dp[i][j];
	
	pair<vector<int>, vector<int>> align_ind = dp_matrix_backtrace(dp, gapopen, gapextend);
	long			alnlen(align_ind.first.size());
	vector<int>		indaln1 = align_ind.first;
	vector<int>		indaln2 = align_ind.second;
	
	string			alnseq1(alnlen, '-');
	string			alnseq2(alnlen, '-');

//	if ( verbose & VERB_DEBUG )
		cout << "DEBUG seq_pair_align:"  << endl << alnseq1.length() << tab << alnseq1 << endl
			<< alnseq2.length() << tab << alnseq2 << endl;

	for ( i=0; i<alnlen; ++i ) {
		if ( indaln1[i] > -1 ) alnseq1[i] = seq1[indaln1[i]];
		if ( indaln2[i] > -1 ) alnseq2[i] = seq2[indaln2[i]];
//		cout << indaln1[i] << tab << alnseq1[i] << tab << indaln2[i] << tab << alnseq2[i] << endl;
	}
	
	pair<string,string>		str2 = make_pair(alnseq1, alnseq2);
...
*/
int			seq_find_best_offset(string seq1, string seq2, 
				long& nres, Bresidue_matrix& simat)
{
	nres = 0;
	if ( seq1.size() < 10 || seq2.size() < 10 ) return 0;
	
	long			offset(0);
	long   			n1(seq1.size()), n2(seq2.size()), n(n1+n2), k, l;
	vector<double>	diag(n, 0);

	Matrix			dot;
	
	dot = seq_dot_plot(seq1, seq2, simat);

	for ( k=0; k<n1; k++ ) {
		for ( l=0; l<n2; l++ ) {
			if ( dot[l][k] > 0 ) diag[n1-l-1+k] += 1;
		}
	}
	...
	if ( verbose & VERB_FULL ) {
		cout << "Offset:                           " << offset << tab << nres << endl;
		if ( offset > 0 ) cout << &seq1[offset] << endl;
		else cout << seq1 << endl;
		if ( offset < 0 ) cout << &seq2[-offset] << endl;
		else cout << seq2 << endl;
		cout << endl;
	}
...
/*Matrix		seq_dot_plot(Bmolecule* mol1, Bmolecule* mol2, Bresidue_matrix& simat)
{
	...
	for ( k=0; k<mol1->nres; k++ ) {
		ii = code1.find(mol1->seq[k]);
		for ( l=0; l<mol2->nres; l++ ) {
			jj = code1.find(mol2->seq[l]);
			if ( ii > -1 && jj > -1 )
	        	dot_plot[l][k] = sim_mat[ii][jj];
        }
	}
...
{
	if ( verbose & VERB_FULL )
	    cout << "Dot plot between sequences of sizes " << seq1.size() << " and " << seq2.size() << endl;
	...
	Matrix			dot_plot(seq1.size(), seq2.size());

	cout << code1.size() << endl;
	cout << sim_mat.rows() << tab << sim_mat.columns() << endl;
	cout << dot_plot.rows() << tab << dot_plot.columns() << endl;
	
	for ( k=0; k<seq1.size(); ++k ) {
//		cout << k << tab << seq1[k] << endl;
		if ( ( ii = code1.find(seq1[k]) ) != string::npos ) {
			for ( l=0; l<seq2.size(); ++l ) {
//				cout << l << tab << seq2[l] << endl;
				if ( ( jj = code1.find(seq2[l]) ) != string::npos )
		        	dot_plot[k][l] = sim_mat[ii][jj];
				else
					cerr << seq2[l] << " not found!" << endl;
			}
        } else {
			cerr << seq1[k] << " not found!" << endl;
		}
	}
