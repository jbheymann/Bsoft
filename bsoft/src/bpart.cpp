...
"-ewald                   Calculate phase difference along Ewald sphere.",
" ",
"Input:",
...
int			main(int argc, char** argv)
{
	...
	bool			ew_flag(0);					// Ewald sphere flag for phase difference calculation
	...
	for ( curropt = option; curropt; curropt = curropt->next ) {
		...
		if ( curropt->tag == "ewald" ) ew_flag = 1;
	...
	if ( pref && phifile.length() ) {
		...
		pphi = project_aberration_phase_difference(project, pref, hires, ew_flag, kernel);
