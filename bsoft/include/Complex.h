...
public:
	...
	bool	is_zero() { return ( fabs(re) < 1e-30 ) && ( fabs(im) < 1e-30 ); }
	bool	not_zero() { return ( fabs(re) > 1e-30 ) || ( fabs(im) > 1e-30 ); }
	Type	real() { return re; }
	Type	imag() { return im; }
	...
	Complex square() { return Complex<Type>(re*re - im*im, 2*re*im); }
